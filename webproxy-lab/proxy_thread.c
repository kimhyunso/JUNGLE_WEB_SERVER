...
const char* host, const char* port, const char* path, char header[][MAXLINE], int num_headers);
// 동시성
...
int main(int argc, char** argv){
    ...
    while(1){ // 무한 루프로 새 클라이언트 연결을 수락
        ...
        *connfdp = Accept(listenfd, (SA*)&clientaddr, &clientlen);
        ...
        pthread_t tid;
        pthread_create(&tid, NULL, worker, connfdp); // 스레드 생성
        pthread_detach(tid); // 스레드 
    }
}
